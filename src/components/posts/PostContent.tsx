...
export function PostContent({ content, images }: { content: string; images: PostImage[] }) {
  return splitPostContentByImages(content, images).map((part, index) => {
    ...
    return (
      <figure key={image.id} className="my-8 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
