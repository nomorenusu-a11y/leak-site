...
const EXTRA_LOCATIONS = [
  ["강남구", "역삼동"], ["수원시 장안구", "정자동"], ["인천 남동구", "간석동"],
  ["강동구", "길동"], ["수원시 권선구", "권선동"], ["인천 부평구", "산곡동"],
  ["강북구", "번동"], ["수원시 팔달구", "인계동"], ["인천 미추홀구", "용현동"],
  ["강서구", "화곡동"], ["성남시 수정구", "신흥동"], ["인천 연수구", "동춘동"],
  ["관악구", "신림동"], ["성남시 중원구", "은행동"], ["인천 서구", "검암동"],
  ["광진구", "구의동"], ["고양시 일산서구", "주엽동"], ["인천 계양구", "계산동"],
  ["구로구", "오류동"], ["고양시 덕양구", "행신동"], ["인천 중구", "신흥동"],
  ["금천구", "시흥동"], ["용인시 기흥구", "구갈동"], ["인천 동구", "송현동"],
  ["노원구", "중계동"], ["용인시 처인구", "역북동"], ["인천 강화군", "강화읍"],
  ["도봉구", "방학동"], ["부천시 소사구", "심곡본동"], ["인천 옹진군", "영흥면"],
  ["동대문구", "답십리동"], ["부천시 오정구", "원종동"],
  ["동작구", "상도동"], ["안산시 단원구", "고잔동"],
  ["마포구", "합정동"], ["안산시 상록구", "본오동"],
  ["서대문구", "홍제동"], ["안양시 만안구", "안양동"],
  ["서초구", "방배동"], ["평택시", "비전동"],
  ["성동구", "행당동"], ["화성시", "오산동"],
  ["성북구", "장위동"], ["시흥시", "배곧동"],
  ["송파구", "가락동"], ["김포시", "장기동"],
...
const EXTRA_PROFILES = [
  { leak: "수도계량기 누수", symptom: "물을 모두 잠갔는데 계량기 별침이 계속 움직임", location: "계량기 이후 세대 직수관과 밸브", check: "수전과 변기·보일러 밸브를 차례로 차단해 계량기 반응을 비교", repair: "누수 계통을 좁힌 뒤 밸브 또는 손상 배관만 보수", keywords: ["수도계량기", "수도배관", "밸브", "탐지장비"] },
  { leak: "난방배관 누수", symptom: "보일러 압력이 떨어지고 특정 바닥만 따뜻함", location: "분배기에서 각 방으로 이어지는 난방 회로", check: "회로별 압력과 바닥 온도·수분 분포를 함께 확인", repair: "손상 회로와 위치를 확인한 뒤 최소 범위로 부분 보수", keywords: ["난방배관", "보일러", "분배기", "바닥"] },
  { leak: "천장 누수", symptom: "아랫집 천장 물자국이 사용 시간에 따라 커짐", location: "위층 욕실·주방과 천장 내부 배관 경로", check: "급수·배수·방수 사용 조건을 나눠 물자국 변화를 확인", repair: "원인 계통을 확정한 뒤 해당 연결부나 배관 구간 보수", keywords: ["천장", "욕실", "배수관", "급수배관"] },
  { leak: "변기 누수", symptom: "변기 주변이 젖거나 물 내린 뒤 아래층에 흔적이 생김", location: "변기 급수호스·물탱크·정심과 바닥 접합부", check: "대기 상태와 물내림 상태를 분리해 젖는 시점을 확인", repair: "급수부와 배수 접합부 가운데 확인된 원인을 재시공", keywords: ["변기", "화장실", "욕실", "배수관"] },
  { leak: "싱크대 누수", symptom: "주방 하부장 안쪽에 물기와 냄새가 반복됨", location: "수전호스·배수트랩·벽체 배수 연결부", check: "급수와 배수를 각각 사용해 물이 생기는 연결부를 확인", repair: "호스·트랩·벽체 연결부 중 손상 부품을 선택 보수", keywords: ["싱크대", "주방", "배수관", "급수배관"] },
  { leak: "욕실 방수 누수", symptom: "샤워 시간이 길었던 날 아래층 벽이나 천장이 젖음", location: "욕실 바닥 방수층·배수구·벽체 모서리", check: "배관 압력 이상을 먼저 제외하고 구역별 살수 반응을 확인", repair: "유입 경계를 좁힌 뒤 필요한 방수 구간만 보수", keywords: ["욕실", "화장실", "바닥", "천장"] },
  { leak: "배수관 누수", symptom: "물을 많이 흘려보낼 때 바닥이나 천장에서 물이 나타남", location: "배수트랩·이음부·공용 수직 배수관", check: "소량과 대량 배수 조건을 나눠 누수와 역류 여부를 확인", repair: "막힘과 관 손상을 구분한 뒤 확인된 배수 구간 보수", keywords: ["배수관", "하수관", "바닥", "천장"] },
  { leak: "온수배관 누수", symptom: "온수 사용 뒤 마루가 따뜻해지고 습기가 올라옴", location: "보일러에서 욕실·주방으로 이어지는 매립 온수관", check: "냉수와 온수 사용 전후 압력·온도·수분 변화를 비교", repair: "온수관 손상 지점을 확인해 최소 굴착으로 배관 보수", keywords: ["온수배관", "보일러", "매립배관", "바닥"] },
  { leak: "창틀 누수", symptom: "비바람이 강한 날 창가 벽과 바닥이 젖음", location: "창틀 코킹·배수홀·외벽 접합부", check: "풍향과 빗물 흔적, 배수홀과 외부 코킹 상태를 함께 확인", repair: "실내 결로를 구분한 뒤 실제 유입 구간을 방수 보수", keywords: ["창틀", "외벽", "벽체", "바닥"] },
  { leak: "옥상 누수", symptom: "비가 온 다음 날 최상층 천장에 얼룩이 번짐", location: "옥상 방수층·파라펫·우수 배수구", check: "균열과 배수 정체, 벽체 접합부의 유입 흔적을 확인", repair: "유입 범위에 맞춰 부분 또는 구간 방수 보수", keywords: ["옥상", "천장", "외벽", "배수관"] },
  { leak: "공용배관 누수", symptom: "여러 세대가 물을 쓸 때 계단이나 공용 벽이 젖음", location: "공용 급수관·수직 배수관과 세대 분기점", check: "세대별 사용 시간과 공용관 압력·배수 반응을 대조", repair: "전용과 공용 구간을 구분해 관리 주체와 보수 범위를 결정", keywords: ["공용배관", "급수배관", "배수관", "벽체"] },
  { leak: "세면대 누수", symptom: "욕실장 안쪽과 바닥에 물기가 반복해서 생김", location: "세면대 수전호스·앵글밸브·배수트랩", check: "냉수·온수·배수를 차례로 사용해 젖는 시점을 확인", repair: "호스·밸브·트랩 중 원인이 확인된 부품을 보수", keywords: ["세면대", "욕실", "밸브", "배수관"] },
] as const;
