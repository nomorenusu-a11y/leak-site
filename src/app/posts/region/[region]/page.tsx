...
export async function generateMetadata({
  ...
  searchParams: Promise<ListSearch>;
}): Promise<Metadata> {
  ...
  const title = `${tag} 누수 시공 사례${filters.page > 1 ? ` · ${filters.page}페이지` : ""}`;
  const description = `${tag} 지역의 누수 탐지·시공 사례 모음. 비파괴 정밀 진단, 1년 무상 A/S.`;
  const canonical = listPath(
    `/posts/region/${cityCodeToSlug(code)}`,
    filters.page,
    filters.category,
  );
    ...
    <>
      ...
      <main className="flex-1 pb-20">
        <section className="border-b border-slate-200 bg-slate-50 py-10 sm:py-14">
          <Container>
            <p className="text-brand-700 text-sm font-semibold">시공 사례</p>
            ...
            {code === "Dobong" && (
              <Link
                href="/seoul/dobong-gu"
                className="text-brand-700 mt-4 inline-block font-semibold underline"
              >
                도봉구 법정동별 누수 안내
              </Link>
            )}
        ...
        <Container className="py-10">
          <PostList
            ...
            basePath={listPath(`/posts/region/${cityCodeToSlug(code)}`, 1, filters.category)}
