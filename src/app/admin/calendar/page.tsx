...
export default async function AdminCalendarPage({
  ...
  for (const post of posts)
    grouped.set(dateKey(post.published_at), [
      ...(grouped.get(dateKey(post.published_at)) ?? []),
      post,
    ]);
  const now = new Date();
  ...
  const scheduledAhead = posts.filter(
    (post) => post.published && new Date(post.published_at) > now,
  ).length;
  const monthPublished = posts.filter(
    (post) =>
      post.published &&
      new Date(post.published_at) <= now &&
      dateKey(post.published_at).startsWith(month),
  ).length;
    ...
    <div className="mx-auto max-w-[1500px]">
      <header className="flex flex-wrap items-end justify-between gap-5">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-400">
            공개된 글과 예약 글을 날짜별로 확인합니다. 예약 시각이 되면 자동으로 공개됩니다.
          </p>
        </div>
      ...
      <section className="mt-7 grid gap-3 sm:grid-cols-3">
        <Summary
          ...
          suffix="개"
          hint={`오늘 전체 배치 ${todayPosts.length}개`}
          tone="emerald"
        />
        <Summary
          label="앞으로 예약"
      ...
      <section className="mt-6 overflow-hidden rounded-2xl border border-white/[0.09] bg-[#0d121d]">
        <div className="flex items-center justify-between border-b border-white/[0.08] px-4 py-4 sm:px-6">
          <Link
            href={`/admin/calendar?month=${monthShift(month, -1)}`}
            aria-label="이전 달"
            className="grid size-10 place-items-center rounded-xl border border-white/10 text-slate-300 hover:bg-white/[0.06]"
          >
            <ChevronLeft size={18} />
          </Link>
          <div className="text-center">
            ...
            <Link
              href="/admin/calendar"
              className="mt-1 inline-block text-xs font-semibold text-blue-300"
            >
              오늘로 이동
            </Link>
          </div>
        ...
        <div className="grid grid-cols-7 border-b border-white/[0.08] bg-white/[0.025] text-center text-[11px] font-bold text-slate-500 sm:text-xs">
          {["일", "월", "화", "수", "목", "금", "토"].map((label) => (
            <div key={label} className="py-3 first:text-rose-300 last:text-blue-300">
              {label}
            </div>
          ))}
        </div>
        <div className="grid grid-cols-7">
          {cells.map((day) => {
            ...
            return (
              <div
                key={key}
                className={`min-h-28 border-r border-b border-white/[0.07] p-1.5 sm:min-h-40 sm:p-2.5 ${inMonth ? "bg-[#0d121d]" : "bg-black/20"}`}
              >
                <div className="flex items-center justify-between">
                  <span
                    className={`grid size-7 place-items-center rounded-full text-xs font-bold ${isToday ? "bg-blue-500 text-white" : inMonth ? "text-slate-300" : "text-slate-700"}`}
                  >
                    {day.getUTCDate()}
                  </span>
                  {dayPosts.length > 0 && (
                    <span className="rounded-full bg-white/[0.06] px-1.5 py-0.5 text-[10px] font-bold text-slate-400">
                      {dayPosts.length}
                    </span>
                  )}
                </div>
        ...
        <div className="flex flex-wrap gap-4 border-t border-white/[0.08] px-5 py-4 text-xs text-slate-400">
          <span className="inline-flex items-center gap-1.5">
            <FileCheck2 size={14} className="text-emerald-300" /> 공개 완료
          </span>
          <span className="inline-flex items-center gap-1.5">
            <Clock3 size={14} className="text-blue-300" /> 공개 예약
          </span>
          <span className="text-slate-600">시간은 한국 표준시(KST) 기준</span>
        </div>
      </section>
    </div>
  );
...
function Summary({
  ...
  return (
    <div className="rounded-2xl border border-white/[0.09] bg-[#0d121d] p-5">
      <span className={`block h-1 w-12 rounded-full bg-gradient-to-r ${color}`} />
      <p className="mt-4 text-xs font-bold tracking-[0.12em] text-slate-500 uppercase">{label}</p>
