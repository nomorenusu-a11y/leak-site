...
import { ArrowUpRight, CalendarDays, CheckCircle2, ClipboardCheck, Images, Sparkles } from "lucide-react";
...
async function loadDashboardData() {
  ...
  const [todayNew, activeOrQuote, publishedPosts, scheduledPosts, draftPosts, taggedMedia, reviewMedia, recentPosts, recentRequests] = await Promise.all([
    ...
    supabase.from("posts").select("*", { count: "exact", head: true }).eq("published", true).lte("published_at", nowIso),
    supabase.from("posts").select("*", { count: "exact", head: true }).eq("published", true).gt("published_at", nowIso),
    ...
    supabase.from("posts").select("id, title, slug, published, published_at, updated_at, region_tags").order("updated_at", { ascending: false }).limit(6),
    supabase.from("leak_requests").select("id, customer_name, region, status, created_at").order("created_at", { ascending: false }).limit(5),
  ]);
  return { counts: { todayNew: todayNew.count ?? 0, activeOrQuote: activeOrQuote.count ?? 0, publishedPosts: publishedPosts.count ?? 0, scheduledPosts: scheduledPosts.count ?? 0, draftPosts: draftPosts.count ?? 0, taggedMedia: taggedMedia.count ?? 0, reviewMedia: reviewMedia.count ?? 0 }, recentPosts: recentPosts.data ?? [], recentRequests: recentRequests.data ?? [] };
}

const quickActions = [
  { href: "/admin/calendar", title: "발행 캘린더", description: "날짜별 공개 수와 예약 글을 한눈에 확인합니다.", icon: CalendarDays, tone: "bg-violet-500/15 text-violet-300" },
...
export default async function AdminDashboardPage() {
  ...
  return <div className="mx-auto max-w-7xl">
    ...
    <section className="mt-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="공개 예약" value={counts.scheduledPosts} hint="예약 시각에 자동 공개" accent="violet" href="/admin/calendar" /><Metric label="공개 글" value={counts.publishedPosts} hint={`전체 운영 콘텐츠 중 ${publishRate}%`} accent="blue" href="/admin/posts?status=published" /><Metric label="사진 자동 선택 가능" value={counts.taggedMedia} hint={`추가 확인 필요 ${counts.reviewMedia}장`} accent="emerald" href="/admin/media" /><Metric label="오늘 신규 견적" value={counts.todayNew} hint={`진행·견적 ${counts.activeOrQuote}건`} accent="amber" href="/admin/requests" /></section>
    <section className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{quickActions.map(({ href, title, description, icon: Icon, tone }) => <Link key={href} href={href} className="group rounded-2xl border border-white/[0.09] bg-white/[0.035] p-5 transition hover:-translate-y-0.5 hover:border-blue-400/40 hover:bg-white/[0.06]"><span className={`grid size-10 place-items-center rounded-xl ${tone}`}><Icon size={19} /></span><div className="mt-5 flex items-start justify-between gap-3"><div><h2 className="font-bold text-white">{title}</h2><p className="mt-1 text-sm leading-6 text-slate-400">{description}</p></div><ArrowUpRight size={18} className="shrink-0 text-slate-600 transition group-hover:text-blue-300" /></div></Link>)}</section>
    <section className="mt-7 grid gap-6 xl:grid-cols-[1.2fr_.8fr]"><div className="overflow-hidden rounded-2xl border border-white/[0.09] bg-[#0d121d]"><div className="flex items-center justify-between border-b border-white/[0.08] px-5 py-4"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">Publish queue</p><h2 className="mt-1 font-bold text-white">최근 작성한 글</h2></div><Link href="/admin/calendar" className="text-sm font-semibold text-blue-300 hover:text-blue-200">캘린더 열기</Link></div><ul className="divide-y divide-white/[0.07]">{recentPosts.length === 0 ? <li className="px-5 py-12 text-center text-sm text-slate-500">아직 만든 글이 없습니다.</li> : recentPosts.map((post) => { const scheduled = post.published && new Date(post.published_at) > new Date(); return <li key={post.id}><Link href={`/admin/posts/${post.id}/edit`} className="flex items-center gap-3 px-5 py-4 transition hover:bg-white/[0.035]"><span className={`size-2 rounded-full ${scheduled ? "bg-blue-400" : post.published ? "bg-emerald-400" : "bg-amber-400"}`} /><div className="min-w-0 flex-1"><p className="truncate font-semibold text-slate-100">{post.title}</p><p className="mt-1 truncate text-xs text-slate-500">{post.region_tags?.join(" · ") || "지역 미지정"} · {formatDateYMD(post.published_at)}</p></div><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${scheduled ? "bg-blue-400/10 text-blue-300" : post.published ? "bg-emerald-400/10 text-emerald-300" : "bg-amber-400/10 text-amber-300"}`}>{scheduled ? "예약" : post.published ? "공개" : "검토 대기"}</span></Link></li>; })}</ul></div><div className="rounded-2xl border border-white/[0.09] bg-[#0d121d] p-5"><p className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">SEO publishing checklist</p><h2 className="mt-1 font-bold text-white">공개 전 5초 점검</h2><ol className="mt-5 space-y-4 text-sm text-slate-300"><Checklist text="지역·건물·누수 유형이 제목과 H1에 일치" /><Checklist text="사진은 본문 흐름에 맞고 ALT·캡션이 존재" /><Checklist text="법정동·구 페이지와 관련 글로 내부링크" /><Checklist text="전화 CTA와 메타 설명 확인" /></ol><Link href="/admin/posts?status=draft" className="mt-6 flex items-center justify-center gap-2 rounded-xl border border-blue-400/30 bg-blue-500/10 px-4 py-3 text-sm font-bold text-blue-200 hover:bg-blue-500/20"><ClipboardCheck size={17} /> 발행 대기함 검토</Link></div></section>
