...
import { buildGuideContent, CAMPAIGN_GUIDES, DAILY_PUBLISH_COUNTS, DAILY_PUBLISH_TIMES, getPublishSlot } from "../src/lib/weekly-content-plan";
...
test("September campaign contains five to ten guides at varied times each day", () => {
  ...
  assert.equal(DAILY_PUBLISH_COUNTS.reduce((sum, count) => sum + count, 0), 111);
